            ...
            .then(async (userDetails) => {
                if (userDetails) {
                    ...
                    properties.map(async (propertyId) => {
                        ...
                        propertyDetails.bidPropertyHistory = [{
                            userId: id,
                    ...
                    //Empty Cart
                    //let propertyIds = Array.from(updatedProperties, ({ propertyId }) => propertyId.toString())
                    userDetails.cartProperties = userDetails.cartProperties.filter(obj => !properties.includes(obj.propertyId.toString()))
 ...
 * @param {Array} properties mongo id of property
 */
 async function sellProperties(id, properties) {
    if (!id || !ObjectId.isValid(id)) { 
        return mapper.responseMapping(
            usrConst.CODE.BadRequest,
            usrConst.MESSAGE.InvalidDetails
        ...
        return dao
            ...
            .then(async (userDetails) => {
                if (userDetails) {
                    ...
                    if(!details.bounds || !details.zoom){
                        return mapper.responseMapping(
                            usrConst.CODE.BadRequest,
                            usrConst.MESSAGE.InvalidDetails
                        );
                    }else{
                        const polygon = details.bounds
                        const res = details.zoom;
                        const hexIds = h3.polyfill(polygon, res);

                        // const chunk = (arr, size) =>
                        //     Array.from({ length: Math.ceil(arr.length / size) }, (v, i) =>
                        //         arr.slice(i * size, i * size + size)
                        //     );
                        // const data = chunk(hexIds, 500000)
                        ...
                        //     properties = await dao.getProperties(propertyQuery)
                        //     findLands.push(properties)
                        // }
                        ...
                        let hexIdsDb = Array.from(properties, ({ hexID }) => hexID)

                        const filteredArray = hexIdsDb.filter(value => hexIds.includes(value));
                        
                      
                        return mapper.responseMappingWithData(
                                usrConst.CODE.Success,
                                usrConst.MESSAGE.Success,
                                {propertyCount: filteredArray.length}
                            );
 ...
 */
 async function getPropertiesInfo(id, hexID) {
    if (!id || !ObjectId.isValid(id) || !hexID) { 
        return mapper.responseMapping(
            usrConst.CODE.BadRequest,
            usrConst.MESSAGE.InvalidDetails
        );
            ...
            .then(async(ud) => {
                ...
                if (ud) {
                    ...
                    if(propertyDetails){
                        ...
                        return mapper.responseMapping(
                            usrConst.CODE.DataNotFound,
                            usrConst.MESSAGE.NoPropertyFound
                        );
                    }
 ...
 */
 async function makeBid(id, propertyId, amount) {
    if (!id || !ObjectId.isValid(id) || !propertyId || !ObjectId.isValid(propertyId)) {
        return mapper.responseMapping(
            usrConst.CODE.BadRequest,
            usrConst.MESSAGE.InvalidDetails
            ...
            .then(async (userDetails) => {
                if (userDetails) {
                    ...
                    if (propertyDetails.propertyStatus != constants.PROPERTY_STATUS.AUCTIONED) {
                        ...
                        let filteredUsers = propertyDetails.bidPropertyHistory.filter(obj => obj.userId != id)
                        let userIds = Array.from(filteredUsers, ({ userId }) => userId)
                        let sendNotificationsUsers = [...new Set(userIds)]
                        console.log({sendNotificationsUsers})
                        sendNotificationsUsers.map(async(userId) => {
                            // Create bell notification
                            let userData = await dao.getUserDetails(userId)
                            console.log({userData})
                            let notificationMessage = `${userDetails.name} has bid on ${propertyDetails.propertyName} property`
                            ...
                            let notificationObject = {
                                ...
                                status: constants.STATUS.ACTIVE,
                                categoryType: constants.NOTIFICATION_CATEGORIES.NEW_BID,
                                refId: propertyDetails._id,
                            }
                            await dao.createNotification(notificationObject)
                            await socketHandler.emitUserNotification(userData.socketId)
        ...
        return dao
            ...
            .then(async (userDetails) => {
                if (userDetails) {
                    ...
                    let sortQuery = {}
                    if(details.filterWith == "lowest_highest"){
                        sortQuery.propertyBoughtAmount=1
                    }
                    if(details.filterWith == "highest_lowest"){
                        sortQuery.propertyBoughtAmount=-1
                    }
                    if(details.filterWith == "recent_buy"){
                        sortQuery.propertySoldOn=-1
                    }
                    

                    let properties = await dao.getProperties(propertyQuery, sortQuery)
        ...
        return dao
            ...
            .then(async (userDetails) => {
                if (userDetails) {
                    ...
                    return dao
                        .updateProperty(propertyQuery, details)
                        .then((updateDone) => {
                            if (updateDone) {

                                return mapper.responseMappingWithData(
                                    usrConst.CODE.Success,
                                    usrConst.MESSAGE.PropertyUpdateSuccess,
                                    updateDone
                                );
                            } else {
        ...
        return dao
            ...
            .then(async (userDetails) => {
                if (userDetails) {
                    ...
                    let propertyQuery = {
                        _id: { $in: propertyIds },
                        propertyCategory: constants.PROPERTY_STATUS.HOT_PROPERTY
                    }

                    let propertyData = await dao.getProperties(propertyQuery)

                    let hotProperty = propertyData.filter(obj => obj.propertyCategory === constants.PROPERTY_STATUS.HOT_PROPERTY)
                    let monumentProperty = propertyData.filter(obj => obj.propertyCategory === constants.PROPERTY_STATUS.MONUMENT_PROPERTY)

                    let propertyStats = {}
                    propertyStats.totalProperties=userDetails.properties.length
                    propertyStats.totalPropertiesStatus=1

                    propertyStats.soldProperties=userDetails.soldProperties.length
                    propertyStats.soldPropertiesStatus=2

                    propertyStats.hotProperties=hotProperty.length
                    propertyStats.hotPropertiesStatus=3

                    propertyStats.monumentProperties=monumentProperty.length
                    propertyStats.monumentPropertiesStatus=4
        ...
        return dao
            ...
            .then(async (userDetails) => {
                if (userDetails) {
                    ...
                    let allUsers = await dao.getAllUsers({})
                    console.log({allUsers})
                    let userStats = {}
                    userStats.totalUsers = allUsers.length
                    userStats.activeUsers = allUsers.filter(obj => obj.status === constants.STATUS.ACTIVE).length
                    userStats.newUsers = allUsers.filter(obj => obj.createdAt > appUtils.getBeforeOneMonthTimestamp()).length

                    let landAnalytics = {}
                    landAnalytics.acquired = {
                        quntity: userDetails.properties.length,
                        coins: 456,
                        amount: "$1451",
                        status: -12.5
                    }
                    ...
                    landAnalytics.favourite = {
                        quntity: userDetails.favouriteProperties.length,
                        coins: 168,
                        amount: "$234",
                        status: 12.5
                    }
                    landAnalytics.sold = {
                        quntity: userDetails.soldProperties.length,
                        coins: 356,
                        amount: "$1875",
                        status: 0.9
                    }
