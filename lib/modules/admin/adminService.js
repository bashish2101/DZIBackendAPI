...
function createProperty(id, details) {
    if (!id || !ObjectId.isValid(id) || !details) {
        ...
        return Promise.all([
            ...
            dao.getPropertyDetails(hexQuery),
        ]).then((results) => {
            console.log({results})
            if (!results[0]) {
                ...
                return mapper.responseMapping(admConst.CODE.BadRequest, admConst.MESSAGE.hexAlreadyExist)
    ...
    if (!id || !ObjectId.isValid(id)) {
        ...
        return dao.getAdminDetails(adminQuery).then(async (adminDetails) => {
            ...
            if (adminDetails) {
                ...
                if(userId){
                    propertyQuery.propertyOwner=ObjectId(userId) 
                }

                if (queryParams.search) {

                    propertyQuery['$or'] = [
                        { 'propertyName': { '$regex': queryParams.search, '$options': 'i' } },
